...
## 进度维护（每次会话都遵守）
- **会话/任务开始**：先读 `PROGRESS.md`，搞清「现在在哪 + 下一步」再动手。
- **完成一个有意义的步骤后**：更新 `PROGRESS.md` 的「现在」和「下一步」；必要时在「决策日志」追加一条（日期 + 为什么，只增不改）。
- `PROGRESS.md` 是活文档，随时变；本文件（CLAUDE.md）是稳定约定，别把易变状态写进来。
